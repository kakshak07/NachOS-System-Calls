...
main(int argc, char **argv)
{
    ...
    if (userProg.size()>0) {
	for(vector<char *>::iterator iter=userProg.begin(); iter!=userProg.end(); ++iter){
		Thread *t = new Thread("user program thread");
		t->Fork((VoidFunctionPtr) RunUserProg, (void *) *iter);
